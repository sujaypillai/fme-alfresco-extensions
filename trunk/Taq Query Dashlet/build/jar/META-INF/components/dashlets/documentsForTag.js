...
(function()
{
   ...
   YAHOO.extend(FME.dashlet.DocumentsForTag, Alfresco.component.Base,
   {
       ...
       */
      onConfigDashletClick: function DocumentsForTag_onConfigFeedClick(e)
      {
         Event.stopEvent(e);
         ...
         if (!this.configDialog)
         {
            ...
            {
               width: "50em",
               templateUrl: Alfresco.constants.URL_SERVICECONTEXT + "modules/dashlet/documentsForTag/config",
               onSuccess:
               {
                  fn: function DocumentsForTag_onConfigDashlet_callback(response)
                  {
                	  ...
                	  this.widgets.dataSource.sendRequest(
                				"&filterData="+ this.options.tag + "&size=" + this.options.rowsPerPage  + "&pos=" +  this.initialPage,
                				{
               ...
               doSetupFormsValidation:
               {
                  fn: function RssFeed_doSetupForm_callback(form)
                  {
                     ...
                     Dom.get(this.configDialog.id + "-tag").value = this.options.tag;
