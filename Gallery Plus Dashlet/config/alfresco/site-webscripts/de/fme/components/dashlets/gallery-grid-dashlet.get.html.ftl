...
(function()
{
   var galleryPlus = new FME.dashlet.GalleryPlus("${idJS}").setOptions({
   		...
   		"title" : "${title?html}",
   		"viewmode" : "${(viewmode!'images')?js_string}",
   		"singleAlbumNodeRef" : "${(singleAlbumNodeRef!'')?js_string}",
   		"filterPath" : "${(filterPath!'')?js_string}",
   		"filterTags" : "${(filterTags!'')?js_string}",
   		"sort" : "${(sort!'cm:name')?js_string}",
   		"sortOrder" : "${(sortOrder!'asc')?js_string}",
   		"pageSize" : "${(pageSize!'50')?js_string}",
   		"maxImages" : "${(maxImages!'100')?js_string}",
