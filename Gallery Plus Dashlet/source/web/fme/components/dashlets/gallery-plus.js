...
(function()
{
   ...
   YAHOO.extend(FME.dashlet.GalleryPlus, Alfresco.component.Base,
   {
      ...
      buildImageRow: function GalleryPlus_buildImageRow(maxwidth, items) {
    	  var row = [];
    	  var len = 0; // ignore first margin
      ...
      updateThumbnailImages: function GalleryPlus_updateThumbnailImages(cleanup) {
      ...
      updateImageElement: function(item) {
      ...
      createImageElement: function(parentNode, item) {
    	  ...
    	  YAHOO.util.Event.addListener(img, 'load', function(ev) {
        	  var imgFade = new YAHOO.util.Anim(img, { opacity: { to: 1 } }, 0.5);    	  
        	  imgFade.animate();
    	  }); 
      ...
      onImageClick: function(e, args) {
         ...
         for (index in this.currentThumbnails) {
        	 if (this.currentThumbnails[index].nodeRef == selectedNodeRef) break;
         }
      ...
      loadImages: function GPD_loadImages(loadmore, selectedAlbum) {
	  ...
	  loadSpecificNumberOfImages : function(loadmore, selectedAlbum, numberOfImages) {
		 if (this.currentlyLoading) return;
      ...
      /**
		 ...
		 * @method onReady
		 */
      onReady: function GalleryPlus_onReady()
      {
    	  var me = this;
      ...
      drawAlbums: function(albums) {
          ...
          for (var a in albums) {
              ...
              }, {gallery: this, album: album}, false);
        	  
              this.widgets.imageArea.appendChild(stack);
          }
      },
      
      fadeOutImageArea : function() {
    	  this.imageAreaAnimation = new YAHOO.util.Anim(this.widgets.imageArea, { opacity: { from:1, to: 0 } }, 1);    	  
            ...
            this.configDialog = new Alfresco.module.SimpleDialog(this.id + "-configDialog").setOptions(
            {
               ...
               doSetupFormsValidation:
               {
                  fn: function DocumentsForTag_doSetupForm_callback(form)
                  {
                     ...
                     form.addValidation(this.configDialog.id + "-maxImages", Alfresco.forms.validation.regexMatch, {pattern: /^\d+$/, match:true}, "keyup", this.msg("message.validation.maxImages"));
                     form.addValidation(this.configDialog.id + "-pageSize", Alfresco.forms.validation.regexMatch, {pattern: /^\d+$/, match:true}, "keyup", this.msg("message.validation.maxImages"));
