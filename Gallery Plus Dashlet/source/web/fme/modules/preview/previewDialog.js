   ...
   YAHOO.extend(FME.module.PreviewDialog, Alfresco.component.Base,
   {
	     /**
	      ...
	      showImage : function(index) {
	    	  if (index !== undefined) {
		    	  if (index < 0) index = 0;
		    	  if (index >= this.options.items.length) index = this.options.items.length - 1; 
	          ...
	          this.widgets.image.src = Alfresco.constants.PROXY_URI + "api/node/"  + 
		         	nodeRefUrl + "/content/thumbnails/galpPreview800?c=force";
	      ...
	      loadComments : function AmSD_loadComments(nodeRef) {

     //     	Dom.setStyle(this.widgets.commentArea, "opacity" , "0.6");
		      ...
		      var url = Alfresco.constants.URL_SERVICECONTEXT + "components/node/" + nodeRefUrl + "/comments?pageSize=1000";
		          ...
		          // Execute the request to retrieve the list of images to
					// display
...
//		                	Dom.setStyle(this.widgets.commentArea, "opacity" , "1");
...
//		                	new YAHOO.util.Anim(this.widgets.commentArea, { opacity: { to: 1 } }, 0.3).animate();    	  
		                },
		                scope: this
		             },
		             failureCallback:
	      ...
	      createComment: function(comment) {
      		if (comment.author.avatarRef) {
				var avatarUrl = Alfresco.constants.PROXY_URI + "api/node/" + 
    					comment.author.avatarRef.replace("://","/") + "/content/thumbnails/avatar?c=force";
    		}
    		else {
	      ...
	      formatCommentDate : function(createdOn) {
	      ...
	      onOutsideDialogClick : function(e) {
	      ...
	      postComment: function AmSD_postComment() {
